...
def run_bonus_vision(
    image: np.ndarray,
    stable_tubes: list[Tube],
    debug_dir: str = "debug/latest",
    save_debug: bool = False
) -> VisionResult:
    """
    High-speed, persistent-geometry vision pipeline for progressive reveal bonus levels.

    Reuses frozen initial tube geometries so that completed, corked, or particle-decorated
    tubes never disappear from the board state.
    """
    return run_vision_pipeline(image, debug_dir=debug_dir, save_debug=save_debug, stable_tubes=stable_tubes)
...
def run_vision_pipeline(
    ...
    """
    Execute the Ball Sort vision pipeline on an in-memory BGR image.

    If stable_tubes is provided, skips contour-based tube discovery and directly uses
    the authoritative frozen tube geometry, providing full immunity against completed/corked
    tube disappearance and dramatically speeding up processing.
    """
    ...
    # ── Stage 4 / Persistent Geometry: Tube Resolution
    if stable_tubes is not None:
        if not stable_tubes:
            raise ValueError("Provided stable_tubes list is empty.")
        tubes = [copy.deepcopy(t) for t in stable_tubes]
        edges = None
        img_contours = None
    else:
    ...
    img_slots = image.copy() if save_debug else None
    img_occupancy = image.copy() if save_debug else None
    img_colors = image.copy() if save_debug else None
    img_combined = image.copy() if save_debug else None
    ...
    if save_debug:
        ...
        edge_frame = cv2.cvtColor(resize_h(edges), cv2.COLOR_GRAY2BGR) if edges is not None else np.zeros((TARGET_H, 200, 3), dtype=np.uint8)
        contour_frame = resize_h(img_contours) if img_contours is not None else np.zeros((TARGET_H, 200, 3), dtype=np.uint8)

        stages = [
            ("ORIGINAL",   resize_h(image)),
            ("EDGES",      edge_frame),
            ("CONTOURS",   contour_frame),
        ...
        if edges is not None:
            saves["edges"] = (edges, "edges.png", "03_edges.png")
        if img_contours is not None:
            saves["contours"] = (img_contours, "contours.png", "04_all_contours.png")
